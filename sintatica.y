...
//lista de tokens
%token TK_MAIN
%token TK_IF TK_TERN TK_ELSE TK_THEN TK_END_LOOP TK_WHILE TK_DO TK_FOR TK_BREAK TK_CONTINUE TK_ENTRADA TK_SAIDA
...
ATRIBUICAO 	: TK_DEC_VAR TK_ID TK_TIPO_CHAR '=' E
			...
			*/
			...
			| TRYING TK_ID ']' '=' E //array[index] = valor
			{
				variable Var = searchForVariable($1.label);
			...
			| TRYING E ']' '=' E //array[expressão] = valor
			{
				variable Var = searchForVariable($1.label);
...
TERNARIO	: TK_TERN {valorLoops++; stackLoops.push(valorLoops);} CONDICIONAL '?' ATRIBUICAO ':' ATRIBUICAO ';'
			{
				if($3.tipo != "bool"){
					yyerror("Condicional sem declaração do tipo booleano!\n");
				}
